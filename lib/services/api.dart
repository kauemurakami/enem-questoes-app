...
class ApiService {
  Future<Either<AppError, List<Exam>>> fetchExams() async {
    final Either<AppError, http.Response> response =
        await handleHttpExceptions(() async => await http.get(Uri.parse('$baseUrl/exams')));
  ...
  Future<Either<AppError, Exam>> fetchExam(String year) async {
    final Either<AppError, http.Response> response =
        await handleHttpExceptions(() async => await http.get(Uri.parse('$baseUrl/exams/$year')));
